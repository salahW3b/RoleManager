...
module.exports = {
    name: "roles",
    description: "Modification des rôles",
    options: [
        {
            name: 'user',
        ...
        if (!db.get(`Owner_${interaction.guild.id}-${interaction.user.id}`) &&
            !db.get(`Wl_${interaction.guild.id}-${interaction.user.id}`) &&
            !config.owners.includes(interaction.user.id) &&
            interaction.user.id !== interaction.guild.ownerId) {
            return interaction.reply({ content: `\`❌\` *Vous n'avez pas les permission pour exécuter cette commande*`, ephemeral: true });
        }

        let member = await interaction.guild.members.fetch(interaction.options.getUser('user').id);
        const pageSize = 6;
        let currentIndex = 0;
